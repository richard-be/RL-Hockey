...
from actor import GaussianPolicy, GaussianPolicyConfig
from feedforward import NNConfig, NormalizationConfig, get_gradient_norm
...
class CrossQConfig:
    ...
    batch_norm_warmup: int = 30_000
    batchn_norm_momentum: int = .01
...
class CrossQAgent:
    ...
    def fit(self, global_step: int) -> None:
        ...
        if self.config.dynamic_alpha:
            ...
            if self.config.use_tensorboard:
                self.writer.add_scalar("Grad/Alpha", self.log_alpha.grad.detach().norm(2).item(), global_step)
        ...
        if self.config.use_tensorboard:
            ...
            for idx, q_func in enumerate(self.q_functions):
                self.writer.add_scalar(f"Grad/Q_{idx}", get_gradient_norm(q_func), global_step)
        ...
        if global_step % self.config.policy_delay == 0:
            ...
            if self.config.use_tensorboard:
                ...
                self.writer.add_scalar("Grad/Actor", get_gradient_norm(self.policy), global_step)
    ...
    def train(self) -> None:
        ...
        for step in range(self.config.train_steps):
            ...
            if step >= self.config.learning_starts:  # wait till we have enough data for the full mini-batch
