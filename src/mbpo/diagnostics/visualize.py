...
from mbpo.util import load_dynamics_model
import hockey
...
class FreezeHockey(Freeze):
    ...
    def __enter__(self):
        self._init_state = self._env.obs_agent_two()
        self._elapsed_steps = self._env.time

    def __exit__(self, *_args):
        self._env.set_state(self._init_state)
        self._env.time = self._elapsed_steps
...
class Visualizer:
    def __init__(
        ...
        self.handler.freeze = FreezeHockey

        self.env, term_fn, reward_fn = self.handler.make_env(self.cfg)

        self.reward_fn = reward_fn

        self.dynamics_model = load_dynamics_model(self.model_path, self.env, self.cfg)
